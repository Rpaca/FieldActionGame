...
{
    public static float MOVE_AREA_RADIUS = 500.0f; // 섬의 반지름.
    ...
    { // 키 조작 정보 구조체.
        ...
        public bool one; // 먹는다 / 수리한다.
        public bool two; // 먹는다 / 수리한다.
        public bool tree; // 먹는다 / 수리한다.
        public bool four; // 먹는다 / 수리한다.
        public bool five; // 먹는다 / 수리한다.
        public bool six; // 먹는다 / 수리한다.
    };
    ...
    private void get_input()
    {
        ...
        this.key.one = Input.GetKeyDown(KeyCode.Alpha1);
        this.key.two = Input.GetKeyDown(KeyCode.Alpha2);
        this.key.tree = Input.GetKeyDown(KeyCode.Alpha3);
        this.key.four = Input.GetKeyDown(KeyCode.Alpha4);
        this.key.five = Input.GetKeyDown(KeyCode.Alpha5);
        this.key.six = Input.GetKeyDown(KeyCode.Alpha6);
    }
    ...
    private void move_control()
    {
        ...
        if (is_moved)
        {
            ...
            // 애니메이션 작동
            animator.SetBool("walkTriger", true);
        ...
        {
            animator.SetBool("walkTriger", false);
        }
    }
    ...
    private void pick_or_drop_control()
    {
        do
        {
            ...
            if (GameObject.Find("GameRoot").GetComponent<Inventory>().checkIsFull())
            { 
                break; // 아무것도 하지 않고 메소드 종료.
            }
            if (this.carried_item == null)
