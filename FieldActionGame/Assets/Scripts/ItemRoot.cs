...
public class ItemRoot : MonoBehaviour
{
    ...
    public GameObject eyePrefab = null; // Prefab 'Iron'
    public GameObject meatPrefab = null; // Prefab 'Plant'
    ...
    public static float RESPAWN_TIME_EYE = 10.0f; // 철광석 출현 시간 상수.
    public static float RESPAWN_TIME_MEAT = 15.0f; // 식물 출현 시간 상수.
    ...
    private float respawn_timer_meat = 0.0f; // 철광석의 출현 시간.
    private float respawn_timer_eye = 0.0f; // 식물의 출현 시간
    ...
    void Start()
    {
        ...
        {
            // 철광석의 출현 포인트를 취득하고, 렌더러를 보이지 않게.
            GameObject eyerespawn = GameObject.Find("EyeRespawn");
            eyerespawn.GetComponent<MeshRenderer>().enabled = false;
        }
 
        if (GameManager.instance.stageLevel == 3)
        {
            // 철광석의 출현 포인트를 취득하고, 렌더러를 보이지 않게.
            GameObject meatrespawn = GameObject.Find("MeatRespawn");
            meatrespawn.GetComponent<MeshRenderer>().enabled = false;
        }
    ...
    void Update()
    {
        ...
        respawn_timer_eye += Time.deltaTime;
        respawn_timer_meat += Time.deltaTime;
        ...
        if (GameManager.instance.stageLevel == 2 && respawn_timer_eye > RESPAWN_TIME_EYE)
        {
            respawn_timer_eye = 0.0f;
            this.respawnEye(); // 식물을 출현시킨다.
        }

        if (GameManager.instance.stageLevel == 3 && respawn_timer_meat > RESPAWN_TIME_MEAT)
        {
            respawn_timer_meat = 0.0f;
            this.respawnMeat(); // 식물을 출현시킨다.
        }
    }
    ...
    // 사과를 출현시킨다.
    public void respawnEye()
    {
        // 사과 프리팹을 인스턴스화.
        GameObject go = GameObject.Instantiate(this.eyePrefab) as GameObject;
        // 사과의 출현 포인트를 취득.
        Vector3 pos = GameObject.Find("EyeRespawn").transform.position;
        // 출현 위치를 조정.
        //pos.y = 1.0f;
        pos.x += Random.Range(-1.0f, 1.0f);
        pos.z += Random.Range(-1.0f, 1.0f);
    ...
    // 사과를 출현시킨다.
    public void respawnMeat()
    {
        // 사과 프리팹을 인스턴스화.
        GameObject go = GameObject.Instantiate(this.meatPrefab) as GameObject;
        // 사과의 출현 포인트를 취득.
        Vector3 pos = GameObject.Find("MeatRespawn").transform.position;
        // 출현 위치를 조정.
        //pos.y = 1.0f;
        pos.x += Random.Range(-1.0f, 1.0f);
        pos.z += Random.Range(-1.0f, 1.0f);
